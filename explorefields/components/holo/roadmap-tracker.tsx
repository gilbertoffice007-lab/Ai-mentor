...
import type { RoadmapStage } from '@/lib/careers-data'
...
export function RoadmapTracker({
  ...
  return (
    <section aria-label="Learning roadmap" className="flex flex-col gap-5">
            ...
            return (
              <li key={s.title} className="flex flex-col items-center gap-2">
                <button
                  ...
                  className="relative grid size-9 place-items-center rounded-full transition-all duration-300"
                  style={{
                ...
                >
                  ...
                  {now && (
                    <span
                      aria-hidden
                      className="absolute size-9 anim-pulse-glow rounded-full"
      ...
      {/* ---------- active stage detail ---------- */}
      <div key={active} className="anim-enter holo-panel holo-corner relative flex flex-col gap-2 p-4">
        <div className="flex items-baseline gap-3">
          ...
          <h4 className="holo-text font-display text-sm font-bold tracking-[0.18em] uppercase" style={{ color: 'var(--hud)' }}>
            {stages[active].title}
          </h4>
        </div>
        <p className="text-[15px] leading-relaxed text-foreground/85">{stages[active].desc}</p>
        <ul className="mt-1 flex flex-wrap gap-1.5">
          {stages[active].skills.map((sk) => (
            <li key={sk} className="holo-chip px-2 py-0.5 font-display text-[9px] tracking-[0.14em] uppercase">
              {sk}
            </li>
          ))}
        </ul>
