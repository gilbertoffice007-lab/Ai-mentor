...
import { pad } from '../../data/careersData';
  ...
  return (
    ...
    >
      <div className="relative mx-auto h-0 w-0">
        ...
        {items.map((item, i) => {
          // Shortest signed distance around ring
          let d = i - selected;
          if (d > n / 2) d -= n;
          if (d < -n / 2) d += n;
          ...
          return (
            <button
              ...
              className="pointer-events-auto absolute top-0 left-0 origin-center cursor-pointer"
              style={{
                width: 'clamp(112px, 15vw, 178px)',
                zIndex: 100 + Math.round(cos * 90),
                transform: `translate(-50%, -50%) translate3d(calc(${sin.toFixed(
                  4
                )} * var(--rx)), calc(${cos.toFixed(4)} * var(--ry)), 0) scale(${scale.toFixed(3)})`,
                opacity,
            ...
            >
              <span
                className="holo-corner relative flex flex-col gap-1.5 px-2.5 py-2.5 text-left rounded-lg"
                style={{
              ...
              >
                ...
                <span
                  ...
                  style={{
                    color: isSel ? 'var(--hud)' : 'color-mix(in oklab, var(--accent) 55%, #e2e8f0)',
