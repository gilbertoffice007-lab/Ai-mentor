...
export const Roadmap: React.FC<RoadmapProps> = ({ onNavigate }) => {
  ...
  const toggleTask = (stageNum: number, taskIndex: number) => {
    setStages(prev => {
      const copy = [...prev];
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      {/* 6 Stage Timeline Accordion Bento Cards */}
        ...
        {stages.map((st) => {
          ...
          return (
            ...
            >
              {/* Header */}
              <div
                id={`stage-header-${st.stageNumber}`}
                onClick={() => setExpandedStage(isExpanded ? 0 : st.stageNumber)}
                className="p-6 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-4 select-none hover:bg-slate-800/20 transition-colors"
              >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${
                        ...
                        isCurrent ? 'bg-indigo-500/10 text-indigo-300 border border-indigo-500/20' : 'bg-slate-800 text-slate-400'
                      }`}>
                ...
                <div className="flex items-center gap-4 self-end sm:self-center">
                  <div className="text-right">
                    <span className="text-xs font-semibold text-slate-300 block">{st.progressPercentage || st.completionPercentage || 0}% Completed</span>
                    <span className="text-[10px] text-slate-500">
                      {st.tasks?.filter((t: any) => t.completed).length || 0}/{st.tasks?.length || 0} Modules
                    </span>
                  </div>
              ...
              {isExpanded && (
                <div className="px-6 pb-6 pt-2 border-t border-slate-800 space-y-6">
                  ...
                  {/* Required Capstone Project Link */}
