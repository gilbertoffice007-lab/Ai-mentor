...
export const supabaseAuthService = {
  ...
  async signUp(
    ...
    try {
      ...
      // Check if email confirmation is required (user has no identities or session is null)
      const isConfirmationRequired = data.session === null && data.user.identities && data.user.identities.length === 0;
      ...
      const userProfile: UserProfile = {
        ...
        domainId: profileData.domainId || 'comp-sci',
        careerId: profileData.careerId || 'fullstack-dev',
        careerTitle: profileData.careerTitle || 'Full Stack Developer',
        ...
        currentStreakDays: profileData.currentStreakDays || 1,
  ...
  async fetchProfile(authUser: User): Promise<UserProfile> {
    ...
    try {
      ...
      if (!error && data) {
        return {
          id: data.id,
          fullName: data.full_name || meta.full_name || 'Student Developer',
          email: data.email || authUser.email || '',
          educationLevel: data.education_level || meta.education_level || 'Undergraduate B.Tech CS',
          currentYear: data.current_year || meta.current_year || '3rd Year (Semester 5)',
          country: data.country || meta.country || 'United States',
          phone: data.phone || meta.phone || '',
          domainId: data.domain_id || 'comp-sci',
          careerId: data.career_id || 'fullstack-dev',
          careerTitle: data.career_title || 'Full Stack Developer',
          currentStage: data.current_stage || 2,
          overallProgress: data.overall_progress || 72,
          totalHoursLearned: data.total_hours_learned || 148,
          currentStreakDays: data.current_streak_days || 14,
          xpPoints: data.xp_points || 2450,
          semesterName: data.semester_name || 'Fall Semester 2026',
          semesterStartDate: data.semester_start_date || '2026-08-01',
          semesterEndDate: data.semester_end_date || '2026-12-15',
          ...
          notifications: [
            {
              ...
              message: 'Stage 2 tasks are queued for your learning roadmap.',
              type: 'task',
              date: '10 mins ago',
              read: false,
            },
    ...
    // Fallback profile synthesized from Auth metadata and local storage
    return {
      id: authUser.id,
      fullName: meta.full_name || authUser.email?.split('@')[0] || 'Student Developer',
      email: authUser.email || 'student@university.edu',
      educationLevel: meta.education_level || 'Undergraduate B.Tech CS',
      currentYear: meta.current_year || '3rd Year (Semester 5)',
